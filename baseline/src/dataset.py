...
from src.utils import load_data, load_data_GAT
...
class Dataset:
    def __init__(self, args, dataset):
        self.adj, self.features, self.graph, self.old_adj, self.links, self.nonzero_nodes \
            = load_data(f'../data/{dataset}/', dataset)
    ...
    def sample_subgraph(self, selection, generate_prior=True, return_nodes=False):
        ...
        # sampled_adj = np.zeros((col_dim, col_dim))
        col_idx = 0
...
class SupDataset:
    def __init__(self, args, dataset):
        ...
        sample_size = int(self.sample_size / (1 + self.negative_sample_size))
        self.binary_label = torch.LongTensor([1]*sample_size + [0]*self.negative_sample_size*sample_size).view(-1, 1)
    ...
    def sample(self, mode, return_nodes=False):
        if mode == 'node':
            ...
            adjs = torch.from_numpy(self.old_adj.todense())
            positive_link, negative_link = self.sample_link()
            neg_link = np.array(negative_link)
            pos_link = np.array(positive_link)
            links = np.concatenate((pos_link, neg_link))
            L = links[:, 0]
            R = links[:, 1]
            # fL = self.features[L]
            # fR = self.features[R]
            # samepled_features, sampled_adj = self.sample_subgraph(positive_link + negative_link)
            nodes = np.array(list(set([n for p in positive_link+negative_link for n in p])))
            sampled_adjs = torch.zeros((nodes.shape[0], nodes.shape[0]))
            for i, n in enumerate(nodes):
                sampled_adjs[i] = adjs[n][nodes]
            sampled_features = self.features[nodes]

            xsorted = np.argsort(nodes)
            Lpos = np.searchsorted(nodes[xsorted], L)
            Lindices = torch.LongTensor(xsorted[Lpos])
            Rpos = np.searchsorted(nodes[xsorted], R)
            Rindices = torch.LongTensor(xsorted[Rpos])
            return (sampled_features, sampled_adjs), (Lindices, Rindices), self.binary_label.reshape((self.binary_label.shape[0]))
        else:
            exit('unknown mode!')
...
class GATDataset:
    def __init__(self, args, dataset):
        self.adj, self.features, self.graph, self.links = load_data_GAT(f'../data/{dataset}/', dataset)
    ...
    def sample(self, mode, return_nodes=False):
        if mode == 'node':
            ...
            Rpos = np.searchsorted(nodes[xsorted], R)
            Rindices = torch.LongTensor(xsorted[Rpos])
            return (Lindices, Rindices), self.binary_label.reshape((self.binary_label.shape[0]))
        else:
            exit('unknown mode!')
